...
std::unique_ptr<std::ofstream> Logger::m_logging_file = nullptr;
...
void Logger::set_log_file(const std::string &file_address, bool recreate)   {
    if (recreate)   {
        m_logging_file = make_unique<ofstream>(file_address);
    }
    else {
        m_logging_file = make_unique<ofstream>(file_address, ios::app);
    }
};
